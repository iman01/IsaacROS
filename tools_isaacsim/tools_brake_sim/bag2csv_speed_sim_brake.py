#!/usr/bin/env python3
"""
(from SIM) Bag ➜ speed_data_sim_brake.csv   (t, cmd_input, meas_speed)

• /cmd_vel            geometry_msgs/msg/Twist        -> linear.x (kept in [-1..1])
• /joint_states       sensor_msgs/msg/JointState     -> joint 'speed' (position[i]) in m/s

Use this for SIM brake tests (same schema as real CSV). Output name is
speed_data_sim_brake.csv to avoid clobbering your real CSV.

Usage
-----
python bag2csv_speed_sim_brake.py  <bag.db3>  [--joint speed]
"""
...
def main():
    ...
    # /joint_states -> speed from named joint
    sp_rows = []
    ...
    # Backward asof so each speed sample uses the latest cmd
    data = pd.merge_asof(speed, cmd, on="t", direction="backward")
    out = bag.with_name("speed_data_sim_brake.csv")
