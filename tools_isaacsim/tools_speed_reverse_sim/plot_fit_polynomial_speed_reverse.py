#!/usr/bin/env python3
"""
Reverse steady-state v=f(u) cubic fits for SIM + REAL.

Inputs (auto):
  - speed_data_sim_reverse.csv (SIM)
  - speed_data.csv             (REAL reverse)

Outputs:
  - poly_speed_sim_reverse.json
  - STATIC_speed_fit_reverse.png
"""
...
SIM = pathlib.Path("speed_data_sim_reverse.csv")
...
if have_sim:
    ...
    if len(st_s) >= 3:
        ...
        (OUT / "poly_speed_sim_reverse.json").write_text(json.dumps(poly_s, indent=2))
...
plt.title("Steady-state throttle → speed (reverse)  (" + " vs ".join(ttl_bits) + ")")
plt.grid(True, ls="--", alpha=0.3); plt.legend(); plt.tight_layout()

png = OUT / "STATIC_speed_fit_reverse.png"
